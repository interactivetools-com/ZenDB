...
$rows = DB::select($baseTable, $conditions, ...$params);

// Get: Returns first matching SmartArray, or empty SmartArray if no results 
$row = DB::get($baseTable, $conditions, ...$params); 

// Count: Returns count of matching rows
$count = DB::count($baseTable, $conditions, ...$params);
...
$affectedRows = DB::update($baseTable, $colsToValues, $conditions, ...$params);

// Delete: Deletes one or more rows and returns the number of rows affected
$affectedRows = DB::delete($baseTable, $conditions, ...$params);

// Query: Executes a SQL query and returns a SmartArray
$rows = DB::query($sqlQuery, ...$params);
```
