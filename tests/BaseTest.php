...
{
    public static function getConfigDefaults(): Config
    {
        $config                     = new Config();
        ...
        $config->connectTimeout     = 2;           // (low timeout for testing) connection timeout in seconds, sets MYSQLI_OPT_CONNECT_TIMEOUT
        $config->readTimeout        = 60;          // read timeout in seconds, sets MYSQLI_OPT_READ_TIMEOUT
        return $config;
    }

    /**
     * Helper to get array of config values for backward compatibility
     */
    ...
    public static function resetTempTestTables(): void
    {
        ...
        $sql = <<<__SQL__
            ...
            CREATE TABLE test_users (num INT PRIMARY KEY AUTO_INCREMENT,
              name VARCHAR(255),
        ...
        $db = DB::$mysqli;
        if (!$db || !$db->multi_query($sql)) {
            throw new RuntimeException("Error inserting records: " . ($db ? $db->error : 'No connection established'));
        }
