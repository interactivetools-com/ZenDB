...
class GetTableNamesTest extends BaseTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        // Create test tables
        DB::$mysqli->query("DROP TABLE IF EXISTS test_gettables_a");
        DB::$mysqli->query("DROP TABLE IF EXISTS test_gettables_b");
        DB::$mysqli->query("DROP TABLE IF EXISTS test__underscore");
        DB::$mysqli->query("CREATE TABLE test_gettables_a (id INT PRIMARY KEY)");
        DB::$mysqli->query("CREATE TABLE test_gettables_b (id INT PRIMARY KEY)");
        DB::$mysqli->query("CREATE TABLE test__underscore (id INT PRIMARY KEY)");

        // Create a view
        DB::$mysqli->query("DROP VIEW IF EXISTS test_gettables_view");
        DB::$mysqli->query("CREATE VIEW test_gettables_view AS SELECT 1 AS val");

        // Create a temp table
        DB::$mysqli->query("CREATE TEMPORARY TABLE test_gettables_temp (id INT)");

        // Create a real table then shadow it with a temp table
        DB::$mysqli->query("DROP TABLE IF EXISTS test_gettables_shadowed");
        DB::$mysqli->query("CREATE TABLE test_gettables_shadowed (id INT PRIMARY KEY)");
        DB::$mysqli->query("CREATE TEMPORARY TABLE test_gettables_shadowed (id INT)");
    ...
    {
        try {
            DB::$mysqli->query("DROP TEMPORARY TABLE IF EXISTS test_gettables_temp");
            DB::$mysqli->query("DROP TEMPORARY TABLE IF EXISTS test_gettables_shadowed");
            DB::$mysqli->query("DROP TABLE IF EXISTS test_gettables_shadowed");
            DB::$mysqli->query("DROP TABLE IF EXISTS test_gettables_a");
            DB::$mysqli->query("DROP TABLE IF EXISTS test_gettables_b");
            DB::$mysqli->query("DROP TABLE IF EXISTS test__underscore");
    ...
    public function testIncludePrefixOption(): void
    {
        ...
        $this->assertContains('gettables_a', $withoutPrefix);
        $this->assertContains('gettables_b', $withoutPrefix);
    }
    ...
    public function testSortsUnderscoreTablesToBottom(): void
    {
        ...
        $this->assertNotFalse($aIndex, 'gettables_a should be in list');
        $this->assertGreaterThan($aIndex, $underscoreIndex, '_underscore should sort after gettables_a');
    }
    ...
    public function testWithDifferentPrefix(): void
    {
        ...
        $this->assertNotContains('gettables_a', $tables);
    }
    ...
    public static function provideInclusionScenarios(): array
    {
        ...
        return [
            ...
            'table a'             => ['gettables_a', true],
            'table b'             => ['gettables_b', true],
            'double underscore'   => ['_underscore', true],

            // Shadowed table - real table still listed
            'shadowed by temp'    => ['gettables_shadowed', true],

            // Views - excluded (BASE TABLE only)
            'view excluded'       => ['gettables_view', false],

            // Temp tables - excluded (not in INFORMATION_SCHEMA)
            'temp table excluded' => ['gettables_temp', false],
